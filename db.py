...
class Database:
    ...
    def add_data(self, table_name, fields, values):
        """
        Cette méthode prend en entrée le nom de la table où les données doivent être ajoutées, les noms des champs et les valeurs à ajouter. La méthode crée une requête SQL qui insère les valeurs spécifiées dans les champs spécifiés de la table, et utilise la méthode execute pour exécuter la requête sur la base de données. Enfin, la méthode utilise commit pour enregistrer les modifications sur la base de données.
        """
    ...
    def get_data(self, table_name, fields, conditions=None):
        """
        Cette méthode prend en entrée le nom de la table, les champs à récupérer et une condition facultative pour filtrer les données. La méthode construit une requête SQL qui sélectionne les champs spécifiés de la table, et utilise la méthode execute pour exécuter la requête sur la base de données. Enfin, la méthode utilise fetchall pour récupérer les lignes sélectionnées et les retourne.
        """
