...
class Movie:
    def __init__(
        ...
        assert (
            isinstance(movie_img, str) and len(movie_img) <= 255 or movie_img is None
        ), "L'attribut 'movie_img' doit être une chaîne de caractères ou None."
        assert (
        ...
        assert (
            isinstance(movie_year, int) and movie_year >= 1895
        ), "L'attribut 'movie_year' doit être un entier supérieur ou égal à 1895"
    ...
    def save(self, db):
        if self.id:
            ...
            db.add_data(
                "movies",
                "movie_api_id, movie_original_title, movie_french_title, movie_original_language, movie_img, movie_description, movie_rating, movie_year",
                [
                    self.movie_api_id,
                    self.movie_original_title,
                    self.movie_french_title,
